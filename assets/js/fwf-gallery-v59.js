(()=>{
  ...
  let overlay,img,cardPreview,items=[],index=0,lastFocus=null,lockedY=0,touchStart=null;
  ...
  function render(){
    ...
    const el=items[index],x=source(el),cardMode=el.dataset.lightboxCard==='true';
    overlay.classList.toggle('lightbox-card-mode',cardMode);
    if(cardMode){
      img.hidden=true;img.removeAttribute('src');
      cardPreview.hidden=false;cardPreview.replaceChildren();
      const card=el.closest('.seo-proof-card');
      if(card){
        const clone=card.cloneNode(true);
        clone.querySelectorAll('[data-lightbox]').forEach(n=>{n.removeAttribute('data-lightbox');n.removeAttribute('data-lightbox-card');n.removeAttribute('role');n.removeAttribute('tabindex');});
        cardPreview.appendChild(clone);
      }
    }else{
      cardPreview.hidden=true;cardPreview.replaceChildren();
      img.hidden=false;img.src=x.src;img.alt=x.alt;
    }
  ...
  function open(el){ensure();items=eligible();index=Math.max(0,items.indexOf(el));lastFocus=document.activeElement;render();overlay.classList.add('open');lockPage();const closeBtn=q('.lightbox-close',overlay);if(!window.matchMedia('(pointer: coarse)').matches){closeBtn?.focus({preventScroll:true})}}
...
/* v59: when Web Design is the active portfolio filter, show the full approved device presentation. */
(()=>{
  ...
  const sync=value=>{
    ...
    webCards.forEach(card=>{
      ...
      if(!img)return;
      const next=img.dataset.webDeviceSrc;
      if(next&&img.getAttribute('src')!==next)img.setAttribute('src',next);
    });
  };
  filters.forEach(btn=>btn.addEventListener('click',()=>sync(btn.dataset.filter)));
  const active=filters.find(btn=>btn.classList.contains('is-active'));
  sync(active?active.dataset.filter:'all');
})();
